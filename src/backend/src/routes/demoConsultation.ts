...
export const demoConsultationRouter = Router();

demoConsultationRouter.post("/demo/consultation", upload.single("audio"), async (req, res, next) => {
  ...
  try {
    ...
    if (parsedRoute.inputType === "text") {
      ...
      res.locals.inputType = "text";
      res.locals.transcript = data.originalTranscript || parsedText.textQuery;
    } else {
      ...
      res.locals.inputType = "audio";
      res.locals.transcript = data.originalTranscript || "";
    }
    ...
    req.log.info(
      {
        event: "DEMO_CONSULTATION_COMPLETED",
        inputType: parsedRoute.inputType,
        latencyMs: Date.now() - start,
      },
      "Demo consultation request completed"
    );
  } catch (error) {
    req.log.error(
      {
        event: "DEMO_CONSULTATION_FAILED",
        latencyMs: Date.now() - start,
      },
      "Demo consultation request failed"
    );
    next(error);
  }
});
